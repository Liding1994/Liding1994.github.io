...
(function($,window) {
      ...
      function pauseAll() {
        ...
        $(document).on('almComplete-shim almDone-shim', function(event, alm) {
            ...
            .on('mouseenter.almComplete', function(event) {
                $(this).find('.post-slides.hidden').removeClass("hidden");
            ...
            .on('mouseleave.almComplete', function(event) {
                $(this).find('.post-slides').addClass("hidden");
        ...
        $('.lang-item > a').text(function() {
            return $(this).attr('hreflang').substr(0,2).toUpperCase(); 
        });
        $('ul.hidden .lang-item-en > a').closest('ul.hidden').removeClass('hidden');
        ...
        $('[data-customaction]').each(function() {
            ...
            command.split(/,\s*/).forEach(function(command) {
                ...
                elem.on(eventSpaceList, function(event) {
                    ...
                    debug('invoking', verb, '(', argArray, ') on', victims);
        ...
        if (email && email.length) {
            ...
            .then(function(response) {
                ...
                if (detail.indexOf('invalid') >= 0)
                    ...
                    $('.mailchimp-generic').removeClass('hidden');
            });
        } else {
            emailField.focus();
        }
    ...
    $('.slider').on('afterChange', function(event, slick, currentSlide){
        ...
        if ($('.slider .single-embed-container').hasClass('slick-active')) {
            $('.single-embed-container.slick-active').find('iframe[src*="vimeo.com"]').each(function () {
                $f(this).api('play');
    ...
    if (!Modernizr.touchevents) {
        ...
        $('.slick-dots').on('mouseenter mouseleave', 'button', function(event) {
            $(this).closest('ul').toggleClass('js', event.type == 'mouseenter');
        });
    }
